...
BOOL addRegEntry() {
  ...
  WCHAR regPath[MAX_PATH] =
      L"System\\CurrentControlSet\\Services\\KmdfHelloWorld";
  SIZE_T imagePathSize = 0;
  DWORD keyValue = 0;
  LSTATUS createKey = ERROR_SUCCESS;

  if (!RegCreateKeyEx(HKEY_LOCAL_MACHINE, regPath, 0, NULL, 0, KEY_ALL_ACCESS,
                      NULL, &phkResult, &lpdwDisposition) == ERROR_SUCCESS) {
    printf("RegCreateKeyEx error: %X", GetLastError());
    return FALSE;
  }

  // TODO: Below needs better error handling.
  // TODO: Below needs better formatting.
  WCHAR driverPath[MAX_PATH] = {0};
  _snwprintf_s(driverPath, MAX_PATH, _TRUNCATE, L"%ws%ws", L"\\??\\",
               KMDF_DRIVER_PATH);

  imagePathSize = (DWORD)(sizeof(wchar_t) * (wcslen(driverPath) + 1));
  (void)RegSetValueEx(phkResult, L"ImagePath", 0, REG_EXPAND_SZ,
                      (const BYTE *)driverPath, imagePathSize);
  keyValue = SERVICE_KERNEL_DRIVER;
  (void)RegSetValueEx(phkResult, L"Type", 0, REG_DWORD, (const BYTE *)&keyValue,
                      sizeof(DWORD));
  keyValue = SERVICE_ERROR_IGNORE; // Don't be noisy.
  (void)RegSetValueEx(phkResult, L"ErrorControl", 0, REG_DWORD,
                      (const BYTE *)&keyValue, sizeof(DWORD));
  keyValue = SERVICE_AUTO_START; // Persistence!
  (void)RegSetValueEx(phkResult, L"Start", 0, REG_DWORD,
                      (const BYTE *)&keyValue, sizeof(DWORD));
...
BOOL loadDriver() {
  ...
  *(FARPROC *)&NtLoadDriver = GetProcAddress(hNtdll, "NtLoadDriver");
  ...
  RtlInitAnsiString(&key, "\\Registry\\Machine\\System\\CurrentControlSet\\Serv"
                          "ices\\KmdfHelloWorld");
